...
local function show_current_working_dir()
	...
	print(string.format("config.current working dir = %s", my_working_dir))
	vim.api.nvim_command("config.cd " .. my_working_dir)
end
...
local function nvim_env_info() -- luacheck: ignore
	...
	----------------------------------------------------------------------------
	print("config.init.lua is loaded!")
	print("config.Neovim RTP(Run Time Path ...)")
	---@diagnostic disable-next-line: undefined-field
	_G.PrintTableWithIndent(vim.opt.runtimepath:get(), 4) -- luacheck: ignore
	print("config.====================================================================")
	print(string.format("config.OS = %s", nvim_config["os"]))
	print(string.format("config.Working Directory: %s", vim.fn.getcwd()))
	print("config.Configurations path: " .. nvim_config["config"])
	print("config.Run Time Path: " .. nvim_config["runtime"])
	print(string.format("config.Plugins management installed path: %s", nvim_config["install_path"]))
	print("config.path of all snippets")
...
local function debugpy_info()
	...
	print(string.format("config.$VIRTUAL_ENV = %s", venv))
	local debugpy_path = nvim_config["python"]["debugpy_path"]
	if _G.IsFileExist(debugpy_path) then
		print("config.Debugpy is installed in path: " .. debugpy_path)
	else
		print("config.Debugpy isn't installed in path: " .. debugpy_path .. "yet!")
	end
	print("config.--------------------------------------------------------------------")
end
...
local function nodejs_info() -- luacheck: ignore
	...
	----------------------------------------------------------------------------
	print(string.format("config.node_path = %s", nvim_config.nodejs.node_path))
	print(string.format("config.vim.g.node_host_prog = %s", vim.g.node_host_prog))
	local js_debugger_path = nvim_config["nodejs"]["debugger_path"]
	if _G.IsFileExist(js_debugger_path) then
		print(string.format("config.nodejs.debugger_path = %s", nvim_config.nodejs.debugger_path))
	else
		print("config.JS Debugger isn't installed! " .. js_debugger_path .. "yet!")
	end
	print(string.format("config.debugger_cmd = %s", ""))
